...
int SimulatorBase::VTI_EXPORT = -1;
...
SimulatorBase::SimulatorBase()
{
	...
	//------2021/3/9---------
	m_enableVTIExport = false;
	m_isInitWriteParticlesVTI = false;
	//-----------------------
...
void SimulatorBase::initParameters()
{
	...
	VTI_EXPORT = createBoolParameter("enableVTIExport", "VTI export", &m_enableVTIExport);
	setGroup(VTI_EXPORT, "Export");
	setDescription(VTI_EXPORT, "Enable/disable VTI export.");
...
void SimulatorBase::initSimulation()
{
	...
	//---------------
	if (m_enableVTIExport)
		initWriteParticlesVTI();
}
...
{	
	...
	std::string vtiExportPath = FileSystem::normalizePath(m_outputPath);
	if (m_enablePartioExport)
		FileSystem::makeDirs(partioExportPath);
	if (m_enableVTKExport)
		FileSystem::makeDirs(vtkExportPath);
	if (m_enableVTIExport)
		FileSystem::makeDirs(vtiExportPath);

	if (m_enableVTIExport && m_isInitWriteParticlesVTI) {
		std::string fileName = "ParticleData";
		fileName = fileName + "_" + std::to_string(m_frameCounter);
		std::string exportFileName = FileSystem::normalizePath(vtiExportPath + "/" + fileName);
		START_TIMING("import vti")
		writeParticlesVTI(exportFileName + ".vti");
		STOP_TIMING_AVG;
	}
	else {
		Simulation* sim = Simulation::getCurrent();
...
void SPH::SimulatorBase::initWriteParticlesVTI()
{
	LOG_INFO << "Initialize WriteParticlesVTI";
	Vector3r scale;
	...
	for (unsigned int i = 0; i < m_scene.boundaryModels.size(); i++)
	{
		...
		{
			scale = m_scene.boundaryModels[i]->scale;
			translation = m_scene.boundaryModels[i]->translation;
			rotation = m_scene.boundaryModels[i]->rotation;
			break;
		}
	}

	Real particleRadius = getScene().particleRadius;
	Real particleDiameter = particleRadius * 2;
	int x = scale[0] / particleDiameter;
	int y = scale[1] / particleDiameter;
	int z = scale[2] / particleDiameter;
	m_voxelSize.push_back(x);
	m_voxelSize.push_back(y);
	m_voxelSize.push_back(z);

	m_voxelPos.resize(x * y * z);
	for (auto attr : attributes) {
		if (attr == string("density")) {
			m_voxelDensity.resize(x * y * z);
		}
		else if (attr == string("velocity")) {
			m_voxelVelocity.resize(x * y * z);
		}
	}
...
#pragma omp for schedule(static)
		for (int i = 0; i <z; ++i) {
			for (int j = 0; j < y; ++j) {
				for (int k = 0; k < x; ++k) {
					int id = i * y * x + j * x + k;
					Vector3r pos(particleRadius + particleDiameter * k - scale[0] / 2, particleRadius + particleDiameter * j - scale[1] / 2, particleRadius + particleDiameter * i - scale[2] / 2);
					m_voxelPos[id] = rotation * pos + translation;
				}
			}
		}
	}
	...
	m_voxelPointsIndex = neighborhoodSearch->add_point_set(&getVoxelPos(0)[0], m_voxelPos.size(), false, true, false, this);
	m_isInitWriteParticlesVTI = true;
}

void SPH::SimulatorBase::writeParticlesVTI(const std::string& fileName)
{
	const unsigned numVoxels = m_voxelPos.size();
	...
	//vti中DataArray中没有范围值
	//Real maxDensity0 = 0.0;
	//for (int fm = 0; fm < nFluids; ++fm) {
	//	Real density = sim->getFluidModel(fm)->getDensity0();
	//	if (density > maxDensity0)
	//		maxDensity0 = density;
	//}
	//Real maxVelocity = sim->getMaxV();
...
#pragma omp for schedule(static)
		for (int i = 0; i < numVoxels; ++i) {
			...
			Real density = 0.0;
			Real velocity = 0.0;
			const Vector3r& xi = getVoxelPos(i);
			...
			for (unsigned int pid = 0; pid < nFluids; pid++)
			{
				...
				for (unsigned int j = 0; j < sim->numberOfNeighbors(m_voxelPointsIndex, pid, i); j++)
				{
					const unsigned int neighborIndex = sim->getNeighbor(m_voxelPointsIndex, pid, i, j);
					const Vector3r& xj = fm_neighbor->getPosition(neighborIndex);
					for (auto attr : attributes) {
						if (attr == string("density")) {
							density += fm_neighbor->getVolume(neighborIndex) * sim->W(xi - xj) * sim->getFluidModel(pid)->getDensity0();
						}
						else if (attr == string("velocity")) {
							v +=  fm_neighbor->getVolume(neighborIndex) * sim->W(xi - xj) * sim->getFluidModel(pid)->getVelocity(neighborIndex);
						}
					}
				}
			}
			for (auto attr : attributes) {
				if (attr == string("density")) {
					m_voxelDensity[i] = density;
				}
				else if (attr == string("velocity")) {
					m_voxelVelocity[i] = sqrtf(v.squaredNorm());
				}
			}
		}
	}
	...
	if (file.is_open()) {
		...
		// 判断int类型大小
		if (sizeof(int) == 8) {
			file << "\" header_type=\"UInt64\">" << endl;
		}
		else {
			file << "\" header_type=\"UInt32\">" << endl;
		}

		file << "<ImageData WholeExtent=\"" << "0 " << m_voxelSize[0] - 1 << " 0 " << m_voxelSize[1] - 1 << " 0 " << m_voxelSize[2] - 1
			<< "\" Origin=\"0 0 0\" Spacing=\"1.0 1.0 1.0\">" << endl;
		file << "<Piece Extent=\"" << "0 " << m_voxelSize[0] - 1 << " 0 " << m_voxelSize[1] - 1 << " 0 " << m_voxelSize[2] - 1 << "\">" << endl;

		//这里有问题：多个Scalar量的DataArray如何在PointData标签中表示？
		if (attributes[0] == string("velocity")) {
			file << "<PointData Scalars=\"velocity\">" << endl;
		}
		else {
			file << "<PointData Scalars=\"density\">" << endl;
		}
		...
		//		}
		//file << "Name=\"density\" format=\"ascii\">" << std::endl;
		//for (float value : m_voxelDensity) {
		//	file << value << " ";
		//}
		...
		for (auto attr : attributes) {
			if (attr == string("density")) {
				...
				file << "Name=\""<<attr<<"\" format=\"binary\">" << endl;
				std::stringstream ss;
				int size = m_voxelPos.size();
				...
				file << base64_encode(reinterpret_cast<const unsigned char*>(dataString.data()), size * sizeof(float) + sizeof(int));
				...
				int size = m_voxelPos.size();
				ss.write(reinterpret_cast<char*>(&size), sizeof(int));
				for (float value : m_voxelVelocity) {
					ss.write(reinterpret_cast<char*>(&value), sizeof(float));
				}
				const std::string& dataString = ss.str();
				file << base64_encode(reinterpret_cast<const unsigned char*>(dataString.data()), size * sizeof(float) + sizeof(int));
		...
		//file << "Name=\"Density\" format=\"binary\" RangeMin=\""<< 0 << "\" RangeMax=\"" << maxDensity0 << "\">" << endl;
		//std::stringstream ss;
		//int size = m_voxelDensity.size();
		//ss.write(reinterpret_cast<char*>(&size), sizeof(int));
		//for (float value : m_voxelDensity) {
		//	ss.write(reinterpret_cast<char*>(&value), sizeof(float));
		//}
		...
		//file << base64_encode(reinterpret_cast<const unsigned char*>(dataString.data()), m_voxelDensity.size() * sizeof(float) + sizeof(int));
		...
		LOG_ERR << "Fail to save vti file";
	}

}

std::string SPH::SimulatorBase::base64_encode(unsigned char const* bytes_to_encode, size_t in_len)
{
	size_t len_encoded = (in_len + 2) / 3 * 4;

	unsigned char trailing_char = '=';

	const char* base64_chars_ = {
...
void SimulatorBase::step()
{
	if (TimeManager::getCurrent()->getTime() >= m_nextFrameTime)
	{
		...
		if (m_enablePartioExport || m_enableVTKExport || m_enableVTIExport)
			particleExport();
